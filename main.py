...
def main():
    print(f"🚀 Apex Terminal Başlatılıyor... Hedef Portföy Tipi: {PORTFOLIO_TYPE}")
    ...
    if data is None or data.empty: 
        print("❌ HATA: yfinance verileri indiremedi. Liste veya ağ bağlantısı kontrol edilmeli.")
        return
    ...
    for s in stocks:
        try:
            ...
            techs[s] = t
            score = (
                (40 if t['regime'] == "TREND" else 0) + 
                (max(1 - t['vol'], 0) * 30) + 
                (min(max(t['mom_60'] / 0.5, 0), 1) * 30)
            )
    ...
    if not selected: 
        print("❌ HATA: Seçilen hisseler için yeterli veri hesaplanamadı.")
        return
    ...
    for s in selected:
        ...
        target.append({
            "code": s, "lot": lot, "weight": weights[s], "price": price,
...
# BİR ÖNCEKİ KOPYALAMADA EKSİK KALAN O KRİTİK İKİ SATIR BURADA:
