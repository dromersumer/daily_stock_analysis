...
def ai_call_with_retry(func, code, max_retries=3):
    ...
    for i in range(max_retries):
        try:
            ...
            # 🟢 ROBUST ERROR CHECK: Hem attribute (status_code/code) hem de string kontrolü
            is_rate_limit = (
                getattr(e, 'code', None) == 429 or 
                getattr(e, 'status_code', None) == 429 or 
                "429" in error_msg or 
                "RESOURCE_EXHAUSTED" in error_msg.upper()
            )
            ...
            if is_rate_limit:
                ...
                time.sleep(delay)
                delay *= 2
                continue # 🟢 Kodun okunabilirliğini artıran explicit continue
            else:
                print(f"🩺 DEBUG AI Error for {code}: {error_msg}")
                return "Sistem Onaylı (AI Çevrimdışı)"

    # 🟢 YANILTICI MESAJ DÜZELTİLDİ
    return "AI Rate Limit (Max Retry)"

def ai_trade_desk_commentary(client, code, order, tech, fund):
    # 🟢 Dependency Injection: Client artık parametre olarak geliyor
    if client is None:
...
def main():
    ...
    # 🟢 GLOBAL CLIENT: Döngü dışında sadece bir kez oluşturulur (Büyük Performans Artışı)
    ai_client = None
    if genai is not None and api_key:
        try:
            ai_client = genai.Client(api_key=api_key)
        except Exception as e:
            print(f"⚠️ AI Client başlatılamadı: {e}")
    ...
    md = f"## 🏦 Dr. Ömer - Apex Terminal v24.0 (Architect Edition)\n"
    ...
    for o in orders:
        ai_msg = ai_trade_desk_commentary(
            ai_client, # 🟢 Tekrar tekrar oluşturmak yerine hazır objeyi yolluyoruz
            o['code'],
            o,
